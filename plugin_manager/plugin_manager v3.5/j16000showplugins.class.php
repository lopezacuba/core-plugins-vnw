...
class j16000showplugins
{
    public function __construct()
    {
		...
		if (  is_object($response) && isset($response->meta->code) && $response->meta->code == 200 ) {
		...
		$rest_api_plugins = array('api_feature_cmf' , 'channelmanagement_framework' , 'channelmanagement_jomres2jomres' , 'channelmanagement_rentalsunited' );

		$api_failure_warning = '';
		if ( $api_test_passed == false ) {
			$api_failure_warning = "<p class='alert alert-warning'> Warning : Connectivity to this system's REST API was tested but failed therefore you will not be able to install plugins which rely on the REST API functionality to work. Please visit Administrator > Jomres > Tools > REST API test for more information.</p>";
		}
        ...
        if ($jrConfig[ 'platform_connected' ] == 1) {
        	unset($rp_array->core_gateway_cheque);
			unset($rp_array->core_gateway_paypal);
			unset($rp_array->core_gateway_stripe);

			unset($rp_array->wire_transfer);
			unset($rp_array->western_union);
			unset($rp_array->shopping_cart);
			unset($rp_array->shopping_cart_module);

			unset($rp_array->commission);
			unset($rp_array->subscriptions);
		}
        ...
        foreach ($remote_plugins as $rp) {
			...
			$api_allows_installion_of_this_plugin = true;
			if ( !$api_test_passed ) {
				if ( in_array( $rp[ 'name' ] , $rest_api_plugins ) ) {
					$api_allows_installion_of_this_plugin = false;
				}
			}

			if (
			( $key_validation->allowed_plugins[0] != "*" && !in_array( $rp[ 'name' ] , $key_validation->allowed_plugins ) )
				|| $api_allows_installion_of_this_plugin == false
			) {
		...
		$output[ 'API_FAILURE_WARNING' ] = $api_failure_warning;
